...
- `fuzzyclock_config.yaml` — single source of daemon config: `dialect`, `font`, `latitude`, `longitude`. Loaded once in `main()` by `_load_config()`. Missing/malformed file → all defaults and after-hours disabled (daemon stays on plain day/night). Unknown dialect/font logs a warning and falls back; missing/non-numeric coords log a warning and disable after-hours but keep the resolved dialect/font.
...
- **Config loading.** `_load_config(path=CONFIG_PATH)` parses `fuzzyclock_config.yaml` with `yaml.safe_load` and returns `(dialect, font, latitude, longitude)`. Unknown dialect/font logs a warning and falls back to the default; missing/non-numeric coords log a warning and become `(None, None)` so after-hours stays off. Called once from `main()` (not at import time) so tests can import the module without filesystem reads or warnings.
- **Font variant.** Stored in module global `FONT_VARIANT` after `_load_config()` returns. `_init_fonts()` threads it into every `load_font(size, variant=...)` call so all four sized fonts come from the same variant. The `fredoka` variant checks `<repo>/fonts/Fredoka.ttf` (vendored) before its apt path, letting users drop a custom TTF without an apt install.
