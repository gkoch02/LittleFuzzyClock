...
- **Font resolution.** `FUZZYCLOCK_FONT` env var, validated against `FONT_VARIANTS` (registry in `fuzzyclock_core.py` parallel to `DIALECTS`); unknown values log a warning and fall back to `dejavu`. Resolved by `_resolve_font()` in `main()` and stored in module global `FONT_VARIANT`, which `_init_fonts()` threads into every `load_font(size, variant=...)` call so all four sized fonts come from the same variant. The `fredoka` variant checks `<repo>/fonts/Fredoka.ttf` (vendored) before its apt path, letting users drop a custom TTF without an apt install.
