...
- `fuzzyClock2.py` — dev/dry-run script. `--dry-run` renders to PNG and works on any Linux/macOS box; `--dialect` picks a phrasing; `--font` and `--frame` override the font variant and border style; `--time HH:MM` pins the clock to a fixed time for previews.
- `fuzzyclock_config.yaml` — single source of daemon config: `dialect`, `font`, `frame`, `latitude`, `longitude`. Loaded once in `main()` by `_load_config()`. Missing/malformed file → all defaults and after-hours disabled (daemon stays on plain day/night). Unknown dialect/font/frame logs a warning and falls back; missing/non-numeric coords log a warning and disable after-hours but keep the resolved dialect/font/frame.
...
- `test_daemon.py` — pure-function tests for `current_mode`, `_sleep_to_next_tick`, `_load_config` (YAML parsing + dialect/font/coordinate validation), and the cross-thread render-retry counter. Imports `fuzzyclock_daemon` directly; the module's hardware imports are guarded so this works on CI without GPIO/EPD.
...
- **Config loading.** `_load_config(path=CONFIG_PATH)` parses `fuzzyclock_config.yaml` with `yaml.safe_load` and returns `(dialect, font, frame, latitude, longitude)`. Unknown dialect/font/frame logs a warning and falls back to the default; missing/non-numeric coords log a warning and become `(None, None)` so after-hours stays off. Called once from `main()` (not at import time) so tests can import the module without filesystem reads or warnings.
- **Font and frame variants.** Stored in module globals `FONT_VARIANT` and `FRAME_VARIANT` after `_load_config()` returns. `_init_fonts()` threads `FONT_VARIANT` into every `load_font(size, variant=...)` call so all four sized fonts come from the same variant. `_resolve_frame(font_variant)` maps `FRAME_VARIANT` to a concrete frame name: when `FRAME_VARIANT == AUTO_FRAME` it looks up the font in `FONT_FRAME_CATEGORY`; otherwise it returns `FRAME_VARIANT` directly. The `fredoka` variant checks `<repo>/fonts/Fredoka.ttf` (vendored) before its apt path, letting users drop a custom TTF without an apt install.
...
- Daemon module-level state (`DIALECT`, `FONT_VARIANT`, `FRAME_VARIANT`, `LATITUDE`, `LONGITUDE`, `AFTER_HOURS_ENABLED`, the four font globals) is populated in `main()`, not at import time, so `import fuzzyclock_daemon` in tests doesn't trigger filesystem reads, warning logs, or a SystemExit on a font-less host. Keep it that way.
